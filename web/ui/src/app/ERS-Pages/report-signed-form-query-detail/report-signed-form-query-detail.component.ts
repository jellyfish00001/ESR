...
@Component({
  selector: 'app-report-signed-form-query-detail',
  templateUrl: './report-signed-form-query-detail.component.html',
  styleUrls: ['./report-signed-form-query-detail.component.scss']
})

export class ReportSignedFormQueryDetailComponent implements OnInit {
...
getFormCodeList() {
  ...
  queryResultWithParam(initial: boolean = false) {
    ...
    this.Service.Post(this.EnvironmentconfigService.authConfig.ersUrl + URLConst.QuerySignedDetailForm, this.queryParam).subscribe((res) => {
      if (res && res.status === 200 && res.body != null) {
        ...
        res.body.data?.map(o => {
          result.push({
            ...
            // untaxamount: o.untaxamount ? o.untaxamount.toLocaleString() : '',
            // taxamount: o.taxamount ? o.taxamount.toLocaleString() : '',
  ...
  download() {
    if (this.queryParam != null) {
      this.Service.doPost(this.EnvironmentconfigService.authConfig.ersUrl + URLConst.DownloadSignedDetailForm, this.queryParam, true).subscribe((res) => {
        this.downloadFlow(res, `${this.translate.instant('SignDetailReport')}-${format(new Date(), 'yyyyMMdd')}.xlsx`);
      });
    }
  }
