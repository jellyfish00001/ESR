...
@Component({
  selector: 'app-bdvirtualdepartment',
  templateUrl: './bdvirtualdepartment.component.html',
  styleUrls: ['./bdvirtualdepartment.component.scss']
})
export class BdvirtualdepartmentComponent {
  //頁面狀態
  isSpinning = false;
  ...
  listTableData: VituralDepartmentInfo[] = [];
  ...
  bdaccountInfoTableColumn: TableColumnModel[] = [
    {
      ...
      columnWidth: '30%',
      align: 'center',
      sortFn: (a: VituralDepartmentInfo, b: VituralDepartmentInfo) =>
  ...
  queryResultWithParam(initial: boolean = false) {
    ...
    this.Service.Post(this.EnvironmentconfigService.authConfig.ersUrl + URLConst.QueryBDVirtualDepartmenttList, this.queryParam).subscribe((res) => {
      if (res && res.status === 200 && res.body != null) {
          this.total = res.body.total;
          let result: VituralDepartmentInfo[] = [];
          res.body.data?.map(o => {
              result.push({
                  id: o.id,
                  company: o.company,
  ...
  addItem(params: any) {
    params.Id = null;
    this.Service.Post(this.EnvironmentconfigService.authConfig.ersUrl + URLConst.AddEditDeleteQueryBDVirtualDepartmenttList, params).subscribe((res) => {
        if (res && res.status === 200 && res.body != null) {
  ...
  deleteRow(id: string=null): void {
    ...
    this.Service.Delete(URLConst.AddEditDeleteQueryBDVirtualDepartmenttList, params).subscribe((res) => {
      if (res && res.status === 200 && res.body != null) {
        if (res.body.status == 1) {
  ...
  onCurrentPageDataChange(listOfCurrentPageData: VituralDepartmentInfo[]): void {
  ...
  DownloadFile() {
    let paramValue = this.queryForm.getRawValue();
        this.Service.doPost(URLConst.DownloadBDVirtualDepartment, this.queryParam, true).subscribe((res) => {
    ...
    DownloadTemplate() {
    this.Service.doPost(URLConst.DownloadBDVirtualDepartment, null, true).subscribe((res) => {
      if (res.size > 0) {
  ...
  filters: UploadFilter[] = [
    ...
    this.Service.Post(URLConst.BatchUploadBDVirtualDepartment, formData).subscribe((res) => {
      if (res && res.status === 200 && res.body != null) {
        if (res.body.status == 1) {
          this.message.success(this.translate.instant('operate-successfully'));
          this.queryResultWithParam();
...
export interface VituralDepartmentInfo {
  id: string;
  company: string;
  deptid: string;
  disabled: boolean;
