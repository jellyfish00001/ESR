...
{
    public class BDExpenseDeptDomainService : CommonDomainService, IBDExpenseDeptDomainService
    {
        private IBDExpenseDeptRepository _BDExpenseDeptRepository;
        private IObjectMapper _ObjectMapper;
        public BDExpenseDeptDomainService(
        ...
        public async Task<Result<List<QueryBDExpenseDeptDto>>> GetPageBDExpenseDept(Request<QueryBDExpenseDeptDto> request){
            Result<List<QueryBDExpenseDeptDto>> result = new Result<List<QueryBDExpenseDeptDto>>(){
                data = new List<QueryBDExpenseDeptDto>()
            };
            ...
            List<QueryBDExpenseDeptDto> resultdata = _ObjectMapper.Map<List<BDExpenseDept>, List<QueryBDExpenseDeptDto>>(query);
        ...
        public async Task<Result<string>> AddBDExpenseDept(QueryBDExpenseDeptDto request, string userId){
            ...
            BDExpenseDept hasExpenseDept = await _BDExpenseDeptRepository.GetBDExpenseDept(request.company, request.deptid, request.isvirtualdept);
            ...
            BDExpenseDept bdExpenseDept = _ObjectMapper.Map<QueryBDExpenseDeptDto, BDExpenseDept>(request);
        ...
        public async Task<Result<string>> EditBDExpenseDept(QueryBDExpenseDeptDto request, string userId){
            Result<string> result = new Result<string>();
            result.status = 2;
            BDExpenseDept bdExpenseDept = await _BDExpenseDeptRepository.GetBDExpenseDept(request.company, request.deptid, request.isvirtualdept);
            if (bdExpenseDept != null) //有找到就更新
            {                
                bdExpenseDept.company = request.company;
                bdExpenseDept.deptid = request.deptid;
                bdExpenseDept.isvirtualdept = request.isvirtualdept;
                bdExpenseDept.muser = userId;
                bdExpenseDept.mdate = System.DateTime.Now;
                result.status = 1;
                result.message = L["SaveSuccessMsg"];
